...
def run_online(device):
    ...
    for l in sys.stdin:
        ...
        xs = nlp_utils.transform_to_array2([[words1, words2]], vocab, with_label=False)
        ...
        print('{}\t{:.4f}\t{}\t{}'.format(answer, score, ' '.join(words1), ' '.join(words2)))
...
def run_batch(device, batchsize=64):
    ...
    def predict_batch(words_batch):
        xs = nlp_utils.transform_to_array2(words_batch, vocab, with_label=False)
